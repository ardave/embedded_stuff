...
pub(crate) type GpsPublisher =
    Publisher<'static, CriticalSectionRawMutex, Result<FitnessTrackerSentence, GPSAcquisitionError>, 8, 4, 1>;
            ...
            if let Some(result) = parser.parse_from_byte(*byte) {
                match result {
                    ...
                    Ok(nmea0183::ParseResult::RMC(Some(rmc))) => {
                        gps_publisher.publish(Ok(FitnessTrackerSentence::MinNav(rmc.into()))).await;
                    }
